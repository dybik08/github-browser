...
import { fetchAdditionalUserRepos } from '../../actions/network_actions';
...
export const RepoDetailsModal: React.FC<RepoDetailsModalProps> = (props) => {
    ...
    const onCollapsePanelPress = async (id: string | string[]) => {
        ...
        if (additionalUserReposSectionName === sections[id as string] && reposData.length === 0) {
            const additionalUserRepos = repository_data && await fetchAdditionalUserRepos(repository_data.owner.repos_url);
